...
const EventForm = () => {
  ...
  const delteAllOperationLogs = (e: PreventDefaultType) => {
    e.preventDefault();
    const result = window.confirm(
      "すべての操作ログをを本当に削除してもいいですか？"
    );
    if (result) {
      dispatch({
        type: operationLogsActionTypes.DELETE_ALL_OPERATION_LOGS,
      });
    }
  };
  return (
